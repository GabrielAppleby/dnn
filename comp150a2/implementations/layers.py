...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        sqrt_of_variance = np.sqrt(sample_var + eps)
        residuals = x - sample_mean
        normalized = residuals / sqrt_of_variance

        recover_scale = normalized * gamma
        recover_shift = recover_scale + beta
        out = recover_shift
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
        ...
        #######################################################################
        sqrt_of_variance = np.sqrt(running_var + eps)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        rands = np.random.uniform(0.0, 1.0, x.shape)
        mask = rands < p
        out = x * mask * 1/p
        #######################################################################
        #                           END OF YOUR CODE                          #
